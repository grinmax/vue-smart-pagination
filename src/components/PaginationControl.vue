    ...
    >
      <li class="pagination-list__li">
        ...
        >
          ...
          <svg :style="[{fill : this.def.arrowsSettings.arrowStyleColor}]" v-else-if="this.def.arrowsSettings.arrowStyle === 'styleArrow-2'" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0" y="0" width="10px" height="10px" viewBox="0 0 284.929 284.929" style="enable-background:new 0 0 284.929 284.929;" xml:space="preserve" class=""><g transform="matrix(-1 1.22465e-16 -1.22465e-16 -1 284.929 284.929)"><g>
              <g>
          ...
          <svg :style="[{fill : this.def.arrowsSettings.arrowStyleColor}]" v-if="this.def.arrowsSettings.arrowStyle === 'styleArrow-3'" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0" y="0" viewBox="0 0 443.52 443.52" style="enable-background:new 0 0 443.52 443.52;" xml:space="preserve" width="12px" height="12px"><g transform="matrix(-1 1.22465e-16 -1.22465e-16 -1 443.52 443.52)"><g>
              <g>
      ...
      <template v-for="(item, index) in settings.array_data">
      ...
      <li class="pagination-list__li">
        <button
                ...
                :class="[
                  {'pagination-list__button_disabled': curIndex === settings.array_data.length - 1},
                  styleArrows()
                ]"
          >
            <svg :style="[{fill : this.def.arrowsSettings.arrowStyleColor}]" v-if="this.def.arrowsSettings.arrowStyle === 'styleArrow-1'" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"
                 id="Capa_1" x="0" y="0" viewBox="0 0 31.49 31.49" style="enable-background:new 0 0 31.49 31.49;"
                 xml:space="preserve" width="15px" height="15px" class="">
              <g>
                <path d="M21.205,5.007c-0.429-0.444-1.143-0.444-1.587,0c-0.429,0.429-0.429,1.143,0,1.571l8.047,8.047H1.111  C0.492,14.626,0,15.118,0,15.737c0,0.619,0.492,1.127,1.111,1.127h26.554l-8.047,8.032c-0.429,0.444-0.429,1.159,0,1.587  c0.444,0.444,1.159,0.444,1.587,0l9.952-9.952c0.444-0.429,0.444-1.143,0-1.571L21.205,5.007z"
...
export default {
  ...
  methods: {
    keydown: function (e) {
      if (e.ctrlKey && e.which === 39 && this.curIndex < this.settings.array_data.length - 1) {
    ...
    styleArrows: function () {
      let arrowStyle = ''
      switch (this.settings.arrowStyle) {
        case 'styleArrow-1':
    ...
    switch_page: function (index) {
      this.curIndex = index
      for (let i = 0; i < this.settings.array_data.length; i++) {
        this.$set(this.settings.array_data[i], 'active', false)
      }
      this.$set(this.settings.array_data[index], 'active', true)
      this.$emit('callMethod', index)
    },
    pagination: function (index) {
      if (index === 0 || index === this.settings.array_data.length - 1) {
        return {
          li: true,
          button: true
        }
        ...
        if (this.curIndex === 0) {
          ...
          } else if (this.curIndex + this.maxButton / 2 + 1 > this.settings.array_data.length) {
            if ((index <= this.curIndex + this.maxButton / 2 - 1) && (index >= this.curIndex - this.maxButton / 2 + 1 - (this.curIndex + Math.round(this.maxButton / 2) - this.settings.array_data.length))) {
              return {
                li: true,
                button: true
              }
            } else if (index === this.curIndex - Math.round(this.maxButton / 2) + 1 - (this.curIndex + Math.round(this.maxButton / 2) - this.settings.array_data.length)) {
              return {
                li: true,
                button: false
              }
